...
class SetterForCatan extends React.Component<SetterForCatanProps, SetterForCatanState> {
  constructor(props: Readonly<{}>) {
    ...
    const initialSpecification = boardSpecifications[initialScenarioName][initialPlayerCount][0];
    const initialBoardGenerator = new Boards.BoardGenerator(initialSpecification);
      ...
      <div className="App">
        <header className="App-body">
          ...
          <RadioGroup
              ...
              onChange={(event: any) => {
                this.generateBoard(boardSpecifications, this.state.scenarioName, event.target.value, this.state.useFishermenOfCatanVariant);
              }}
              row
          >
            {playerCounts.map((playerCount) => {
              return (
                  <FormControlLabel
                      ...
                      value={playerCount}
                      label={playerCount}
                      disabled={!boardSpecifications[this.state.scenarioName].hasOwnProperty(playerCount)}
          ...
          <Chip
              ...
              onClick={() => {
                this.generateBoard(boardSpecifications, this.state.scenarioName, this.state.playerCount, !this.state.useFishermenOfCatanVariant)
              }}
          />
          <br/>
          <Tooltip title="Right click to change configuration.">
            ...
            >
              <Typography variant="h4">Generate {this.state.scenarioName}</Typography>
            </Button>
          </Tooltip>
          <Menu
              id="scenarios"
  ...
  async componentDidMount(): Promise<void> {
    console.log("################################################### generating async board");
  
    const board = await BoardRestClient.generateBoard(this.state.specification);
  ...
  generateBoard(boardSpecifications: BoardSpecifications, scenarioName: string, playerCount: string, useFishermenOfCatanVariant: boolean) {
    const noFishSpecification = boardSpecifications[scenarioName][playerCount][0];
    const withFishSpecification = Specifications.withFisheries(noFishSpecification, boardSpecifications[scenarioName][playerCount][1]);
